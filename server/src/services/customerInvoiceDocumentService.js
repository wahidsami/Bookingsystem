...
async function renderCustomerInvoicePdf(invoiceRecord, kind = 'invoice') {
    ...
    await new Promise((resolve, reject) => {
        ...
        const paymentBreakdown = Array.isArray(loadedInvoice?.metadata?.paymentBreakdown)
            ? loadedInvoice.metadata.paymentBreakdown
            : (Array.isArray(loadedInvoice?.paymentMethodSnapshot?.paymentBreakdown) ? loadedInvoice.paymentMethodSnapshot.paymentBreakdown : []);

        let breakdownCardBottom = cursorY + 160;
        if (paymentBreakdown.length > 0) {
            const breakdownY = cursorY + 164;
            const breakdownHeight = Math.max(84, 50 + (paymentBreakdown.length * 22));
            doc.roundedRect(margin, breakdownY, pageWidth - (margin * 2), breakdownHeight, 16).fillAndStroke('#ffffff', border);
            doc.fillColor(text).font('Helvetica-Bold').fontSize(11).text('Payment breakdown', margin + 14, breakdownY + 14);

            let breakdownRowY = breakdownY + 34;
            paymentBreakdown.forEach((item) => {
                const amount = Number(item.amount || 0);
                const isRefund = amount < 0 || `${item.status || ''}`.toLowerCase() === 'refunded';
                const label = `${item.type || 'payment'}`.toLowerCase() === 'refund'
                    ? 'Refund'
                    : `${item.type || 'payment'}`.toLowerCase() === 'deposit'
                        ? 'Deposit'
                        : `${item.type || 'payment'}`.toLowerCase() === 'remainder'
                            ? 'Remainder'
                            : 'Payment';
                const methodLabel = formatPaymentMethodLabel(item.paymentMethod);
                doc.fillColor(text).font('Helvetica').fontSize(9).text(`${label} - ${methodLabel}`, margin + 14, breakdownRowY, { width: pageWidth - (margin * 2) - 150 });
                doc.fillColor(isRefund ? '#b91c1c' : accent).font('Helvetica-Bold').fontSize(9).text(formatMoney(amount, currency), pageWidth - margin - 110, breakdownRowY, { width: 100, align: 'right' });
                breakdownRowY += 18;
            });

            breakdownCardBottom = breakdownY + breakdownHeight;
        }

        const footerY = Math.max(breakdownCardBottom + 24, 730);
