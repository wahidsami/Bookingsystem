...
const { buildSubscriptionInvoiceSnapshot } = require('./invoiceSnapshotBuilder');
const { generateBillNumber, generatePaymentToken } = require('./billUtils');
...
async function checkExpiringSubscriptions() {
    try {
        ...
        for (const subscription of expiringSubscriptions) {
            ...
            // At T-3, T-1, or T-0, auto-generate a renewal invoice if one doesn't exist
            if (daysLeft <= 3) {
                try {
                    const existingRenewalBill = await db.Bill.findOne({
                        where: {
                    ...
                    if (!existingRenewalBill) {
                        ...
                        const invoiceSnapshot = await buildSubscriptionInvoiceSnapshot({
                            tenant,
                            subscriptionPackage: packageObj,
                            subscription,
                            billingCycle,
                            billType: 'renewal',
                            dueDate,
                        ...
                        const paymentTokenExpiresAt = new Date(subscription.currentPeriodEnd);
                        // Add some buffer to token expiry if desired, or leave it exact. The user said dueDate = currentPeriodEnd.
                        // Let's set the token to expire 48 hours after currentPeriodEnd (grace period)
                        paymentTokenExpiresAt.setHours(paymentTokenExpiresAt.getHours() + 48);
                        ...
                        await db.Bill.create({
                            ...
                            metadata: {
                                ...
                                paymentUrl,
                                requestedPackageId: packageObj.id,
                                requestedBillingCycle: billingCycle,
                                requestedAmount: invoiceSnapshot.amount,
                                autoGenerated: true
                            }
                        });
                        
                        console.log(`[Billing] Auto-generated T-3 renewal invoice for tenant ${tenant.id}`);
                    }
                } catch (billError) {
                    console.error(`[Billing] Failed to auto-generate renewal invoice for tenant ${subscription.tenantId}:`, billError);
                }
            }

            // Check if alert already sent for this specific day window
