...
export function AppointmentActionDrawer({
  ...
  const selectedGuestServicesBaseTotal = useMemo(
    () =>
      toSafeMoneyNumber(
        selectedGuestServices.reduce((sum, service) => sum + toSafeMoneyNumber(service.finalPrice || 0), 0)
      ),
    [selectedGuestServices]
  );
  const selectedGuestServicesDiscountTotal = toSafeMoneyNumber(
    Math.max(0, selectedGuestServicesBaseTotal - selectedGuestServicesSubtotal)
  );
  ...
  const guestServicesSummaryLabel = includeGroupGuest
    ? (locale === "ar"
      ? `${selectedGuestServices.length} خدمة إضافية`
      : `${selectedGuestServices.length} guest services`)
    : "";
                  ...
                  <div className="h-full overflow-y-auto pr-1">
                    ...
                    <div className="rounded-[24px] border border-dashed border-gray-200 px-6 py-8 text-center">
                      ...
                      <p className="mt-2 text-sm leading-6 text-gray-500">
                        {locale === "ar"
                          ? "ابدأ بعميل موجود أو انتقل مباشرة إلى الحجز الحضوري."
                          : "Start with an existing client or switch directly to a walk-in booking."}
                      </p>
                    </div>

                    <div className="mt-5 rounded-[22px] border border-gray-200 bg-gray-50 p-4">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-gray-500 ring-1 ring-gray-200">
                          {locale === "ar" ? "وضع الحجز" : "Booking mode"}
                        </span>
                        <span
                        ...
                        {includeGroupGuest ? (
                          <span className="rounded-full bg-fuchsia-50 px-3 py-1 text-xs font-semibold text-fuchsia-700 ring-1 ring-fuchsia-200">
                            {locale === "ar" ? "ضيف إضافي مفعّل" : "Guest included"}
                          </span>
                        ) : null}
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-between rounded-[22px] border border-violet-200 bg-violet-50 px-5 py-4 text-left transition hover:border-violet-300 hover:bg-violet-100"
                    >
                      <div className="flex items-center gap-4">
                        <div className="flex h-12 w-12 items-center justify-center rounded-full border border-violet-200 bg-white text-violet-700">
                          <svg className="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" aria-hidden="true">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 5v14m7-7H5" />
                          </svg>
                        </div>
                        <div>
                          <p className="text-base font-semibold text-gray-900">{locale === "ar" ? "عميل حضوري" : "Walk-in customer"}</p>
                          <p className="mt-1 text-sm text-gray-600">
                            {locale === "ar"
                              ? "إنشاء حجز بدون عميل محفوظ."
                              : "Create a booking without a stored customer."}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="mt-4 rounded-[22px] border border-fuchsia-200 bg-fuchsia-50 p-4">
                      <label className="flex items-start gap-3 text-sm font-semibold text-gray-900">
                        ...
                        <span className="leading-5">
                          <span className="block">{locale === "ar" ? "إضافة ضيف إضافي" : "Include guest"}</span>
                          <span className="mt-1 block text-xs font-normal text-fuchsia-700">
                            {locale === "ar"
                              ? "افتح كتلة مستقلة للضيف والخدمات المرتبطة به."
                              : "Open an independent guest block with its own services."}
                          </span>
                        </span>
                      </label>
                    </div>
                  ...
                  <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary/70">
                          {includeGroupGuest
                            ? (locale === "ar" ? "خدمات العميل الأساسي" : "Main customer services")
                            : (locale === "ar" ? "الخدمات" : "Customer services")}
                        </p>
                        <h4 className="mt-1 text-lg font-semibold text-gray-900">
                          {hasQueuedServices
                    ...
                    {includeGroupGuest ? (
                      <div className="rounded-3xl border border-fuchsia-200 bg-fuchsia-50 p-4 shadow-sm">
                        ...
                        <div className="mt-3 rounded-2xl border border-fuchsia-200 bg-white/80 px-4 py-3 text-xs text-fuchsia-800">
                          <div className="flex flex-wrap items-center justify-between gap-2">
                            <span className="font-semibold">
                              {locale === "ar" ? "خدمات الضيف مستقلة عن خدمات العميل الأساسي" : "Guest services are independent from the main customer"}
                            </span>
                            <span className="rounded-full bg-fuchsia-50 px-2.5 py-1 font-semibold text-fuchsia-700 ring-1 ring-fuchsia-200">
                              {guestServicesSummaryLabel}
                            </span>
                          </div>
                        </div>
                                    ...
                                    <div className="border-t border-fuchsia-100 bg-gray-50 p-4 sm:p-5">
                                      <div className="flex items-start justify-between gap-3">
                                        <div>
                                          ...
                                          <h4 className="mt-1 text-lg font-semibold text-gray-900">{serviceName}</h4>
                                        </div>
                                        <button
                                          type="button"
                      ...
                      {appointmentStep === 3 && hasQueuedServices ? (
                        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm">
                          ...
                          <div className="mt-4 rounded-2xl border border-gray-200 bg-gray-50 p-4">
                            <div className="flex items-center justify-between gap-3 text-sm">
                              <span className="text-gray-600">{locale === "ar" ? "الخدمات الأساسية" : "Main services"}</span>
                              <span className="font-semibold text-gray-900"><Currency amount={queuedServicesBaseTotal} /></span>
                            </div>
                            <div className="mt-2 flex items-center justify-between gap-3 text-sm">
                              <span className="text-gray-600">{locale === "ar" ? "خصم الخدمات الأساسية" : "Main services discount"}</span>
                              <span className="font-semibold text-gray-900">
                            ...
                            {includeGroupGuest ? (
                              <>
                                <div className="mt-2 flex items-center justify-between gap-3 text-sm">
                                  <span className="text-gray-600">{locale === "ar" ? "خدمات الضيف" : "Guest services"}</span>
                                  <span className="font-semibold text-gray-900">
                                    {groupGuest.isFree ? (locale === "ar" ? "مجاني" : "Free") : <Currency amount={selectedGuestServicesSubtotal} />}
                                  </span>
                                </div>
                                <div className="mt-2 flex items-center justify-between gap-3 text-sm">
                                  <span className="text-gray-600">{locale === "ar" ? "خصم الضيف" : "Guest discount"}</span>
                                  <span className="font-semibold text-gray-900">
                                    {groupGuest.isFree
                                      ? (locale === "ar" ? "مجاني" : "Free")
                                      : (selectedGuestServicesDiscountTotal > 0 ? <Currency amount={selectedGuestServicesDiscountTotal} /> : (locale === "ar" ? "بدون" : "None"))}
                                  </span>
                                </div>
                              </>
                            ) : null}
